      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'docker tag nodeapp-dev:trunk tlamin/nodeapp-dev:latest'
            sh 'docker push tlamin/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk tlamin/nodeapp-prod:latest'
                            sh 'docker push tlamin/nodeapp-prod:latest'
                            sh 'docker save tlamin/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
